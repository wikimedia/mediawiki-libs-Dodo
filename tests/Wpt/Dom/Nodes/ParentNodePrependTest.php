...
class ParentNodePrependTest extends WptTestHarness
{
    public function testPrepend($node, $nodeName)
    {
        ...
        $this->assertTest(function () use(&$node) {
            ...
            $parent->prepend(null);
            $this->assertEqualsData($parent->childNodes[0]->textContent, 'null');
            $this->assertEqualsData($parent->childNodes[1], $child);
        ...
        $this->assertTest(function () use(&$node) {
            ...
            $this->assertEqualsData($parent->childNodes[0], $x);
            $this->assertEqualsData($parent->childNodes[1]->textContent, 'text');
            $this->assertEqualsData($parent->childNodes[2], $child);
