...
class DomstringmapSupportedPropertyNamesTest extends WPTTestHarness
{
    public function testDomstringmapSupportedPropertyNames()
    {
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertArrayEquals($this->getOwnPropertyNames($element->dataset), ['']);
        }, 'Object.getOwnPropertyNames on DOMStringMap, empty data attribute');
        $this->assertTest(function () {
            $element = $this->doc->querySelector('#edge2');
            $this->wptAssertArrayEquals($this->getOwnPropertyNames($element->dataset), ['id-']);
        }, 'Object.getOwnPropertyNames on DOMStringMap, data attribute trailing hyphen');
        $this->assertTest(function () {
            $element = $this->doc->querySelector('#user');
            $this->wptAssertArrayEquals($this->getOwnPropertyNames($element->dataset), ['id', 'user', 'dateOfBirth']);
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertArrayEquals($this->getOwnPropertyNames($element->dataset), ['uniqueId', 'middleName']);
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertArrayEquals($this->getOwnPropertyNames($element->dataset), ['uniqueId', 'age']);
