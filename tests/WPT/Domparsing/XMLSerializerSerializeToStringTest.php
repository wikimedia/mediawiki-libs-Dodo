...
class XMLSerializerSerializeToStringTest extends WPTTestHarness
{
    ...
    public function testXMLSerializerSerializeToString()
    {
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<root><child1>value1</child1></root>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root->ownerDocument), '<html><head/><body><div/><span/></body></html>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<root><another xmlns="urn:foo"><child1 xmlns="">value1</child1></another></root>');
        }, 'Check if the default namespace is correctly reset.');
        $this->assertTest(function () {
            $root = parse('<root xmlns="urn:bar"><outer xmlns=""><inner>value1</inner></outer></root>');
            $this->wptAssertEquals(serialize($root), '<root xmlns="urn:bar"><outer xmlns=""><inner>value1</inner></outer></root>');
        }, 'Check if there is no redundant empty namespace declaration.');
        // https://github.com/w3c/DOM-Parsing/issues/47
        $this->assertTest(function () {
            $this->wptAssertEquals(serialize(parse('<root><child xmlns=""/></root>')), '<root><child/></root>');
            $this->wptAssertEquals(serialize(parse('<root xmlns=""><child xmlns=""/></root>')), '<root><child/></root>');
            $this->wptAssertEquals(serialize(parse('<root xmlns="u1"><child xmlns="u1"/></root>')), '<root xmlns="u1"><child/></root>');
        ...
        $this->assertTest(function () use(&$XMLNS_URI) {
            ...
            $this->wptAssertEquals(serialize($root), '<root xmlns="uri1"><child xmlns=""/><child2 xmlns="uri2"/><child3/><child4 xmlns="uri4"/><child5 xmlns=""/></root>');
        }, 'Check if inconsistent xmlns="..." is dropped.');
        $this->assertTest(function () {
            $root = parse('<r xmlns:xx="uri"></r>');
            $root->setAttributeNS('uri', 'name', 'v');
            $this->wptAssertEquals(serialize($root), '<r xmlns:xx="uri" xx:name="v"/>');
            $root2 = parse('<r xmlns:xx="uri"><b/></r>');
            $child = $root2->firstChild;
            $child->setAttributeNS('uri', 'name', 'v');
            $this->wptAssertEquals(serialize($root2), '<r xmlns:xx="uri"><b xx:name="v"/></r>');
            $root3 = parse('<r xmlns:x0="uri" xmlns:x2="uri"><b xmlns:x1="uri"/></r>');
            $child3 = $root3->firstChild;
            $child3->setAttributeNS('uri', 'name', 'v');
            $this->wptAssertEquals(serialize($root3), '<r xmlns:x0="uri" xmlns:x2="uri"><b xmlns:x1="uri" x1:name="v"/></r>', 'Should choose the nearest prefix');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<el1 xmlns:p="u1" xmlns:q="u1"><el2 xmlns:q="u2" q:name="v"/></el1>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root2), '<r xmlns:xx="uri"><b xx:name="value"/></r>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<r xmlns:xx="uri" xmlns:ns1="uri2" ns1:name="value"/>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertInArray(serialize($root), ['<root attr="&#9;"/>', '<root attr="&#x9;"/>']);
            $root->setAttribute('attr', "\n");
            $this->wptAssertInArray(serialize($root), ['<root attr="&#xA;"/>', '<root attr="&#10;"/>']);
        ...
        $this->assertTest(function () use(&$XMLNS_URI) {
            ...
            $this->wptAssertEquals(serialize($root), '<root xmlns:ns1="uri1" ns1:foobar="value1" xmlns:p="uri2"/>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<root xmlns:p="uri1"><child xmlns:ns1="uri2" ns1:foobar="v"/></root>');
        }, 'Check if attribute serialization takes into account of the same prefix declared in an ancestor element');
        $this->assertTest(function () {
            $this->wptAssertEquals(serialize(parse('<root><child/></root>')), '<root><child/></root>');
            $this->wptAssertEquals(serialize(parse('<root xmlns="u1"><p:child xmlns:p="u1"/></root>')), '<root xmlns="u1"><child xmlns:p="u1"/></root>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<root xmlns:p1="u1"><child xmlns:p2="u1"><p2:child2/></child></root>');
        }, 'Check if start tag serialization finds an appropriate prefix.');
        $this->assertTest(function () use(&$XMLNS_URI) {
            $root = (new Document())->createElementNS('uri1', 'p:root');
            $root->setAttributeNS($XMLNS_URI, 'xmlns:p', 'uri2');
            $this->wptAssertEquals(serialize($root), '<ns1:root xmlns:ns1="uri1" xmlns:p="uri2"/>');
        ...
        $this->assertTest(function () use(&$XMLNS_URI) {
            ...
            $root->appendChild($child);
            $this->wptAssertEquals(serialize($root), '<root xmlns:p="uri2"><p:child xmlns:p="uri1"/></root>');
        ...
        $this->assertTest(function () {
            $this->wptAssertEquals(serialize(parse('<root xmlns:x="uri1"><table xmlns="uri1"></table></root>')), '<root xmlns:x="uri1"><x:table xmlns="uri1"/></root>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<root><child1 xmlns:ns1="uri1" ns1:attr1="value1" xmlns:ns2="uri2" ns2:attr2="value2"/><child2 xmlns:ns3="uri3" ns3:attr3="value3"/></root>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<root xmlns:ns2="uri2"><child xmlns:ns1="uri1" xmlns:ns1="uri3" ns1:attr1="value1"/></root>');
        }, 'Check if "ns1" is generated even if the element already has xmlns:ns1.');
        $this->assertTest(function () {
            $root = (new Document())->createElement('root');
            $root->setAttributeNS('http://www.w3.org/1999/xlink', 'href', 'v');
            $this->wptAssertEquals(serialize($root), '<root xmlns:ns1="http://www.w3.org/1999/xlink" ns1:href="v"/>');
            $root2 = (new Document())->createElement('root');
            $root2->setAttributeNS('http://www.w3.org/1999/xlink', 'xl:type', 'v');
            $this->wptAssertEquals(serialize($root2), '<root xmlns:xl="http://www.w3.org/1999/xlink" xl:type="v"/>');
        ...
        $this->assertTest(function () {
            ...
            $this->wptAssertEquals(serialize($root), '<div xmlns="http://www.w3.org/1999/xhtml"></div><span xmlns="http://www.w3.org/1999/xhtml"></span>');
